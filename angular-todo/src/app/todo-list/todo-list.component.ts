...
export class TodoListComponent implements OnInit {
  ...
  public readonly filterTypes: TodoFilterType[] = [
    // TODO: update the database to contain categories independently and get them
    { name: "By Default", isHidden: true },
    { name: "Label" },
    { name: "Description" },
    { name: "Category" },
  ];
  ...
  public getTodoList(): void {
    ...
    this.todoService
      ...
      .subscribe((todos) => {
        ...
        this.todoCategories = [
          ...new Set(this._todoList.map((t) => t.category)),
        ]; // making the taken categories unique
      });
  }
  ...
  public editTodoItem(todoItem: TodoItem): void {
    this.todoService
      ...
      .subscribe(() => {
        // TODO: read the last todo value from service call
  ...
  private _handleSearch(searchValue: string): void {
    ...
    this._todoList.forEach((l: TodoItem) => {
      l.isHidden =
        l[searchType].toLocaleLowerCase().indexOf(searchValue) === -1; // search on todo property based on filter type
    });
  }
}
